...
import { CheckOutline, CloseOutline, PayCircleOutline, SetOutline, UnorderedListOutline } from "antd-mobile-icons";
import './style.css';
  ...
  return (
    ...
    <>
      <div className="sidemenu-container">
        ...
        <ProfileCard avatar={session?.user?.image || '/wiishy.png'} name={session?.user?.name || 'Wiishy user'} />
        
        <List header='Menu' className='mt-5'>
          <List.Item className='text-xs' prefix={<UnorderedListOutline />} onClick={() => { }}>
            Home
          </List.Item>
          <List.Item className='text-xs' prefix={<PayCircleOutline />} onClick={() => { }}>
            Event
          </List.Item>
          <List.Item className='text-xs' prefix={<SetOutline />} onClick={() => { }}>
            Search
          </List.Item>
          <List.Item className='text-xs' prefix={<SetOutline />} onClick={() => { }}>
            Followers
        ...
        <List header='Setting' className='mt-5' style={{fontSize:'13px'}}>
          <List.Item className='text-xs' extra={
            <Switch
